...
audio_out = I2S(0,
                ...
                rate=44100,
                ibuf=buflen)
#half the amplitude 16383 32767
def generate_sine_wave(frequency, amplitude=32767, rate=44100):
    # Calculate the number of samples for one period
    length = int(rate // frequency)
    
    # Create a buffer to store the waveform data
    buf = bytearray(2 * length)  

    for i in range(length):
        # Generate the sine wave value
        val = int(amplitude * math.sin(2 * math.pi * i / length))
...
for note in midi_data:
    ...
    buf = generate_sine_wave(frequency)
